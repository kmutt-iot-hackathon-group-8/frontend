...
const EventEdit = () => {
    ...
    const [startDate, setStartDate] = useState<Date | null>(eventData.eventStart);
    const [endDate, setEndDate] = useState<Date | null>(eventData.eventEnd);
    const [startTime, setStartTime] = useState(formatTime(eventData.eventStart));
    const [endTime, setEndTime] = useState(formatTime(eventData.eventEnd));
    const [registerOpen, setRegisterOpen] = useState<Date | null>(eventData.registrationStart);
    const [registerClose, setRegisterClose] = useState<Date | null>(eventData.registrationEnd);
    const [description, setDescription] = useState(eventData.fullDescription);
    const [contact, setContact] = useState(eventData.contact.contactInformation);
    ...
    const handleSave = () => {
        console.log('Saving event:', {
            title, imagePreview, startDate, endDate, startTime, endTime,
            registerOpen, registerClose, description, contact, registerLink
        });
        navigate(-1);
    };
    return (
                ...
                <div className="flex flex-col xl:flex-row gap-12 lg:gap-20">
                    <div className="shrink-0">
                        <div className="relative w-4/5 aspect-[1.18] sm:w-115 sm:h-97.5 rounded-3xl shadow-lg bg-linear-to-b from-[#AFEEDD] to-[#6CB2D7] flex items-center justify-center group cursor-pointer hover:shadow-xl transition-all mx-auto xl:mx-0 overflow-hidden">
                            {imagePreview ? (
                                <img src={imagePreview} alt="Event" className="absolute inset-0 w-full h-full object-cover" />
                            ) : (
                    ...
                    <div className="grow flex flex-col gap-6 sm:gap-8 w-full">
                        ...
                        <div className="w-full">
                            ...
                            <div className="grid grid-cols-2 xl:grid-cols-4 gap-4 sm:gap-6">
                                <div className="order-1">
                                    <InputWithIcon 
                                        label="Start Date" 
                                        icon={<CalendarDays className="w-5 h-5 text-[#1BB3A9]" />} 
                                        value={startDate}
                        ...
                        <div className="grid grid-cols-2 xl:grid-cols-2 gap-4 sm:gap-6 w-full xl:w-1/2">
                            <InputWithIcon 
                                ...
                                icon={<CalendarDays className="w-5 h-5 text-[#1BB3A9]" />}
                                value={registerOpen}
                                onChange={setRegisterOpen}
                            />
                            <InputWithIcon 
                                label="Register Close" 
                                icon={<CalendarDays className="w-5 h-5 text-[#1BB3A9]" />}
                                value={registerClose}
                                onChange={setRegisterClose}
                            />
                        </div>
                    </div>
                </div>
                ...
                <div className="mt-8 sm:mt-12 flex flex-col xl:flex-row gap-8 sm:gap-12 lg:gap-20">
                    ...
                    <div className="grow flex flex-col justify-between gap-8">
                        ...
                        <div className="flex justify-start sm:justify-end gap-4 sm:gap-6 mt-4 sm:mt-12">
                            ...
                            <button 
                                onClick={handleSave}
                                className="flex-1 sm:flex-none sm:w-46.75 h-13 bg-linear-to-r from-[#20D4A4] to-[#1F7CAE] text-white text-lg sm:text-xl font-bold rounded-xl shadow-lg hover:shadow-xl hover:-translate-y-0.5 transition-all"
                            >
                                Save Changes
                            </button>
                        </div>
...
const InfiniteTimePicker = ({ label, value, onChange }: { label: string; value: string; onChange: (time: string) => void }) => {
    ...
    const handleScroll = (e: React.UIEvent<HTMLDivElement>, type: 'hour' | 'minute') => {
        ...
        if (type === 'hour') {
            ...
            const currentH = value.split(':')[0];
            if (currentH !== h) {
                onChange(`${h}:${value.split(':')[1]}`);
            }
        } else {
            const m = minutesOriginal[normalizedIndex];
            const currentM = value.split(':')[1];
            if (currentM !== m) {
                onChange(`${value.split(':')[0]}:${m}`);
            }
        }
    };
                ...
                <div className="absolute z-50 top-full mt-2 left-0 w-full bg-white border border-gray-100 shadow-2xl rounded-2xl overflow-hidden p-4 animate-in fade-in zoom-in-95 duration-200">
                    <div className="flex justify-between items-center h-48 relative">
                        ...
                        >
                            ...
                            {hoursLooped.map((h, i) => (
                                <div key={i} className={`h-10 flex items-center justify-center snap-center cursor-pointer transition-all duration-150 ${value.split(':')[0] === h ? 'text-[#10726F] font-bold text-xl scale-110' : 'text-gray-400 opacity-60'}`}>
                                    {h}
                                </div>
                            ))}
                            <div className="h-20"></div>
                        ...
                        >
                            ...
                            {minutesLooped.map((m, i) => (
                                <div key={i} className={`h-10 flex items-center justify-center snap-center cursor-pointer transition-all duration-150 ${value.split(':')[1] === m ? 'text-[#10726F] font-bold text-xl scale-110' : 'text-gray-400 opacity-60'}`}>
                                    {m}
                                </div>
                            ))}
                            <div className="h-20"></div>
...
const InputWithIcon = ({ label, icon, value, onChange }: InputProps) => {
    return (
        <div className="w-full">
            ...
            <DatePicker 
                ...
                dayClassName={(date) => value && date.getDate() === value.getDate() && date.getMonth() === value.getMonth() ? "bg-[#54B0A8] text-white font-bold rounded-lg hover:bg-[#4a9b94]" : "text-gray-700 hover:bg-[#AFEEDD] hover:text-[#10726F] rounded-lg transition-colors"}
            />
        </div>
    );
};
