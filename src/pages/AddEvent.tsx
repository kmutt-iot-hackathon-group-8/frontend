...
const AddEvent = () => {
  ...
  const [formData, setFormData] = useState({
    ...
    regisStart: new Date(),
    regisEnd: new Date(),
    eventDetail: "", // This is for the "Brief Details"
    contact: "",
    regisURL: "",
  });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await API.events.create({
        eventOwner: user.id,
        eventName: formData.title, // Map title to eventName
        eventDetail: formData.eventDetail, // Fixed: now maps the detail field
        eventIMG: null,
  ...
  return (
    <div className="min-h-screen bg-white px-4 sm:px-8 lg:px-16 py-6 sm:py-12 pb-32 font-sans overflow-x-hidden">
      ...
      <div className="max-w-400 mx-auto">
        ...
        <form onSubmit={handleSubmit} className="flex flex-col xl:flex-row gap-12 lg:gap-20">
          ...
          <div className="grow flex flex-col gap-6 sm:gap-8 w-full">
            ...
            <div className="w-full">
              ...
              <div className="grid grid-cols-2 xl:grid-cols-4 gap-4 sm:gap-6">
                <div>
                  <label className="block text-sm sm:text-base font-semibold mb-1.5 text-gray-600">Start Date</label>
                  <DatePicker
                    selected={formData.eventStartDate}
                    onChange={(date) => setFormData({ ...formData, eventStartDate: date || new Date() })}
                    dateFormat="MMMM d, yyyy"
                    className="w-full h-13 sm:h-16 bg-white border-2 border-gray-200 rounded-2xl px-4 text-base sm:text-lg text-gray-800"
                  />
                </div>
                <div>
                  <label className="block text-sm sm:text-base font-semibold mb-1.5 text-gray-600">End Date</label>
                  <DatePicker
                    selected={formData.eventEndDate}
                    onChange={(date) => setFormData({ ...formData, eventEndDate: date || new Date() })}
                    dateFormat="MMMM d, yyyy"
                    className="w-full h-13 sm:h-16 bg-white border-2 border-gray-200 rounded-2xl px-4 text-base sm:text-lg text-gray-800"
                  />
                </div>
                <div>
                  <label className="block text-sm sm:text-base font-semibold mb-1.5 text-gray-600">Start Time</label>
                  <input
                    type="time"
                ...
                <div>
                  <label className="block text-sm sm:text-base font-semibold mb-1.5 text-gray-600">End Time</label>
                  <input
                    type="time"
            ...
            {/* Registration Dates */}
            <div className="grid grid-cols-2 gap-4 sm:gap-6 w-full xl:w-1/2">
              <div>
                <label className="block text-sm sm:text-base font-semibold mb-1.5 text-gray-600">Register Open</label>
                <DatePicker
                  selected={formData.regisStart}
                  onChange={(date) => setFormData({ ...formData, regisStart: date || new Date() })}
                  dateFormat="MMMM d, yyyy"
                  className="w-full h-13 sm:h-16 bg-white border-2 border-gray-200 rounded-2xl px-4 text-base sm:text-lg text-gray-800"
                />
              </div>
              <div>
                <label className="block text-sm sm:text-base font-semibold mb-1.5 text-gray-600">Register Close</label>
                <DatePicker
                  selected={formData.regisEnd}
                  onChange={(date) => setFormData({ ...formData, regisEnd: date || new Date() })}
                  dateFormat="MMMM d, yyyy"
                  className="w-full h-13 sm:h-16 bg-white border-2 border-gray-200 rounded-2xl px-4 text-base sm:text-lg text-gray-800"
                />
              </div>
            ...
            <div className="flex flex-col gap-6">
              ...
              <div className="w-full">
                ...
                <input
                  ...
                  onChange={(e) => setFormData({ ...formData, regisURL: e.target.value })}
