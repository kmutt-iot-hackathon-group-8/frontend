...
// --- WIFI SETTINGS ---
const char* primary_ssid = "SIT-HACKATHON";
const char* primary_password = "sithackathon";
const char* fallback_ssid = "iPhone";
const char* fallback_password = "31415926pi";
// --- PINS ---
...
void drawIdleScreen() {
  ...
  tft.setTextFont(4); 
  tft.drawString("Ready to Scan", 160, 145);
  
  tft.setTextColor(C_SUBTEXT, C_CARD);
  tft.setTextFont(2); 
  tft.drawString("Tap your NFC card to check in", 160, 175);
  
  // Add a small status indicator
  tft.setTextColor(C_SUBTEXT, C_BG);
  tft.setTextFont(1);
  tft.drawString("Event #1", 160, 210);
}
...
void drawProcessingScreen() {
  ...
  tft.setTextColor(C_SUBTEXT, C_BG);
  tft.setTextFont(2);
  tft.drawString("Checking card with server", 160, 160);
}
...
void drawWelcomeScreen(String message) {
  ...
  tft.setTextFont(4);
  tft.drawString("CHECKED IN", 160, 130);
  ...
  if (message.indexOf('\n') != -1) {
    ...
    String line1 = message.substring(0, newlinePos);
    String line2 = message.substring(newlinePos + 1);
    
    tft.drawString(line1, 160, 160);
    tft.drawString(line2, 160, 180);
  } else {
    // Single line
    tft.setTextFont(4);
    tft.drawString(message, 160, 170);
  }
}
...
void drawUnregisteredScreen() {
  ...
  tft.setTextDatum(MC_DATUM);
  tft.setTextFont(4);
  tft.drawString("Welcome!", 160, 80);
  
  tft.setTextColor(C_SUBTEXT, C_BG);
  tft.setTextFont(2);
  tft.drawString("This card is not registered yet.", 160, 110);
  tft.drawString("Register to check in to events.", 160, 130);

  btnAction.initButton(&tft, 160, 185, 200, 50, C_PRIMARY, C_PRIMARY, C_TEXT, "Show QR Code", 1);
...
void drawQRScreen() {
  ...
  tft.setTextDatum(MC_DATUM);
  tft.setTextFont(4);
  tft.drawString("Register Your Card", 160, 25);
  
  tft.setTextFont(2);
  tft.drawString("Scan this QR code with your phone", 160, 50);
  tft.drawString("to complete registration", 160, 65);
  ...
  tft.setTextFont(2);
  tft.setTextDatum(MC_DATUM);
  tft.drawString("Tap Screen to Close", 160, 223);
}

void pollCardStatus() {
  String url = "https://iot2026-api.adorio.space/api/v1/card-status/" + currentCardId;
  http.begin(client, url);
  int httpCode = http.GET();
  if (httpCode == 200) {
    String payload = http.getString();
    DynamicJsonDocument doc(1024);p
    DeserializationError error = deserializeJson(doc, payload);
    if (!error) {
      const char* status = doc["status"];
      if (strcmp(status, "present") == 0) {
        Serial.println("[POLL] Registration complete!");
        drawWelcomeScreen("Registration Complete!");
        playTone(TONE_SUCCESS);
        currentState = STATE_RESULT_SHOW;
        resultTimer = millis();
      } else {
        Serial.println("[POLL] Status: " + String(status));
      }
    }
  } else {
    Serial.println("[POLL] Error: " + String(httpCode));
  }
  http.end();
}
...
void setup() {
  ...
  Serial.print("[WIFI] Connecting to primary: "); Serial.println(primary_ssid);
  WiFi.begin(primary_ssid, primary_password);
  while (WiFi.status() != WL_CONNECTED && millis() - startTime < 10000) { // 10 seconds
    delay(500); Serial.print(".");
  }

  if (WiFi.status() == WL_CONNECTED) {
    connected = true;
    Serial.println("\n[WIFI] Connected to primary! IP: " + WiFi.localIP().toString());
  } else {
    Serial.println("\n[WIFI] Primary failed, trying fallback...");
    tft.drawString("Trying fallback...", 160, 140); // Update display
    ...
    if (WiFi.status() == WL_CONNECTED) {
      connected = true;
      Serial.println("\n[WIFI] Connected to fallback! IP: " + WiFi.localIP().toString());
    } else {
      Serial.println("\n[WIFI] Failed to connect to any network!");
      tft.drawString("WiFi Failed!", 160, 140);
      delay(2000); // Show error briefly
    }
  }

  if (!connected) {
    drawErrorScreen("No WiFi Connection");
    playTone(TONE_NETWORK_ERR);
    while(1);
  }
  ...
  if (!versiondata) {
    ...
    playTone(TONE_NETWORK_ERR); // Play sad sound if hardware faails
    while(1);
  }
    ...
    case STATE_IDLE:
      {
        ...
        if (nfc.readPassiveTargetID(PN532_MIFARE_ISO14443A, &uid[0], &uidLength)) {
          ...
          String url = "https://iot2026-api.adorio.space/api/v1/scan-card/" + cardID + "?eventid=1";
          ...
          if (httpCode == 200) {
            ...
            if (!error) {
                ...
                if (type && strcmp(type, "REGISTRATION_COMPLETE") == 0) {
                drawWelcomeScreen(message ? message : "Success!");
                playTone(TONE_SUCCESS);
                currentState = STATE_RESULT_SHOW;
                resultTimer = millis();
                } 
                // Handle other JSON types here if needed...
            } else {    
              // Smart response parsing
                if (payload.startsWith("http://") || payload.startsWith("https://")) {
                // QR Code URL for registration
                Serial.println("[STATUS] New Card - Registration Required");
                playTone(TONE_ATTENTION);
                qrPayload = payload; 
                currentCardId = cardID;
                currentState = STATE_UNREGISTERED;
                drawUnregisteredScreen();
                
                } else if (payload.startsWith("WELCOME_")) {
                // Successful check-in
                String userName = payload.substring(8); // Remove "WELCOME_"
                if (userName.length() == 0) userName = "Guest";
                Serial.println("[STATUS] Check-in Successful: " + userName);
                drawWelcomeScreen("Welcome\n" + userName);
                playTone(TONE_SUCCESS);
                currentState = STATE_RESULT_SHOW;
                resultTimer = millis();
                
                } else if (payload.startsWith("ALREADY_CHECKED_IN_")) {
                // Already checked in
                String userName = payload.substring(19); // Remove "ALREADY_CHECKED_IN_"
                if (userName.length() == 0) userName = "User";
                Serial.println("[STATUS] Already Checked In: " + userName);
                ...
                Serial.println("[STATUS] Invalid Card ID");
                drawErrorScreen("Invalid Card Format");
                playTone(TONE_ACCESS_DENIED);
                currentState = STATE_RESULT_SHOW;
                resultTimer = millis();
                
                } else if (payload == "MISSING_EVENT_ID") {
                Serial.println("[STATUS] Missing Event ID");
                drawErrorScreen("Event Not Specified");
                playTone(TONE_ACCESS_DENIED);
                currentState = STATE_RESULT_SHOW;
                resultTimer = millis();
                
                } else if (payload == "LINK_SUCCESSFUL") {
                  Serial.println("[STATUS] Check-in Successful!");
                  drawWelcomeScreen("Check-in Successful!");
                  playTone(TONE_SUCCESS);
                  currentState = STATE_RESULT_SHOW;
                  resultTimer = millis();
                } else {
                // Unknown response - treat as error
                Serial.println("[STATUS] Unknown Response: " + payload);
                drawErrorScreen("Unknown Error\nContact Support");
                playTone(TONE_NETWORK_ERR);
                currentState = STATE_RESULT_SHOW;
                resultTimer = millis();
                }
            ...
            ToneType errorTone = TONE_NETWORK_ERR;
